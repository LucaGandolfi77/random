...
const FAST_CHAPTER_PLANNER_PROMPT_FILE = 'chapter_planner_fast.txt';
...
export async function generateChapter(rootPath, options) {
  return withActionLog(rootPath, {
    ...
    const deepReview = shouldUseDeepReview(options);
    const reviewMode = deepReview ? 'deep' : 'fast';
    const maxRevisionRounds = deepReview
      ? Number(options.maxRevisions || approval.max_revision_rounds || 1)
      : 0;
    ...
    if (deepReview) {
      ...
      architect = await runAgent(rootPath, state, env, 'architect', baseInput, options.dryRun);
      emitProgress('agent-done', { agent: 'architect', chapter });

      emitProgress('agent-start', { agent: 'character_master', chapter });
      characterMaster = await runAgent(rootPath, state, env, 'character_master', {
        ...baseInput,
        ARCHITECT: architect
      }, options.dryRun);
      ...
      chapterPlanner = await runAgent(rootPath, state, env, 'chapter_planner', {
        ...baseInput,
        ARCHITECT: architect,
      ...
      emitProgress('agent-start', { agent: 'chapter_planner', chapter, merged: true });
      const mergedPlan = await runAgent(
        rootPath,
        state,
        env,
        'chapter_planner',
        baseInput,
        options.dryRun,
        { promptFile: FAST_CHAPTER_PLANNER_PROMPT_FILE }
      );
      ({ architect, chapterPlanner } = buildFastPlannerArtifacts(mergedPlan, chapter, options));
      emitProgress('agent-done', { agent: 'chapter_planner', chapter, merged: true });

      emitProgress('agent-start', { agent: 'character_master', chapter });
      characterMaster = buildFastCharacterMaster(state.bookBible, architect);
      emitProgress('agent-done', { agent: 'character_master', chapter, synthetic: true });
    }
    ...
    if (deepReview) {
      while (!usedWriterFallback && !forcedCompletionReason && revisionRound <= maxRevisionRounds) {
        ...
        try {
          ...
          critic = await runAgent(rootPath, state, env, 'critic', reviewInput, options.dryRun);
          ...
          ({ continuity, votes } = buildSkippedReviewBundle(
            writer,
            chapterPlanner,
            'Deep review now runs critic and editor only; continuity and voting are skipped.',
            { critic, editor, continuity, votes }
          ));
        } catch (error) {
      ...
      ({ critic, continuity, editor, votes } = buildSkippedReviewBundle(
        writer,
        chapterPlanner,
        'Fast mode skips critic, continuity, editor, and voting. Pass --deep-review to enable critic and editor review.',
        { critic, continuity, editor, votes }
      ));
    }
    ...
    const result = {
      message: forcedCompletion
        ? `Chapter ${chapter} generated with forced fallback.`
        : reviewMode === 'deep'
          ? `Chapter ${chapter} generated with deep review.`
          : `Chapter ${chapter} generated in fast mode.`,
      activeBookId: state.activeBook.id,
      chapter,
      chapterTitle,
      reviewMode,
    ...
    emitProgress('chapter-done', { chapter, chapterTitle, revisionRound, reviewMode, approved: votes?.summary?.approved, forced: forcedCompletion });
...
export function shouldRequestRevision(critic, continuity, votes, approval) {
  ...
  return Boolean(criticTooWeak || criticRejected);
}
...
export function shouldUseDeepReview(options = {}) {
  return Boolean(
    options.deepReview
    || options.strictReview
    || options.fullReview
    || options.legacyPipeline
    || options.reviewMode === 'deep'
    || options.reviewMode === 'full'
    || options.mode === 'deep'
  );
}

export function buildFastPlannerArtifacts(mergedPlan = {}, chapter, options = {}) {
  const chapterGoal = String(
    mergedPlan.chapter_goal
    || mergedPlan.chapter_hook
    || options.idea
    || `Advance the conflict in chapter ${chapter}.`
  ).trim();
  const chapterHook = String(mergedPlan.chapter_hook || chapterGoal).trim();
  const arcTargets = toStringList(mergedPlan.arc_targets);
  const nextArcTargets = toStringList(mergedPlan.next_arc_targets);
  const nextChapterSeed = toStringList(mergedPlan.next_chapter_seed);
  const outlineNotes = appendUniqueStrings([], [
    ...toStringList(mergedPlan.outline_notes),
    ...toStringList(mergedPlan.outline_memory_updates)
  ]);
  const cliffhanger = String(
    mergedPlan.cliffhanger
    || nextChapterSeed[0]
    || nextArcTargets[0]
    || ''
  ).trim();
  const sceneCards = Array.isArray(mergedPlan.scene_cards) && mergedPlan.scene_cards.length
    ? mergedPlan.scene_cards.map((card, index) => ({
      scene: Number(card?.scene || index + 1),
      goal: String(card?.goal || chapterGoal).trim(),
      conflict: String(card?.conflict || chapterHook).trim(),
      turn: String(card?.turn || cliffhanger || `Escalate pressure for chapter ${chapter + 1}.`).trim(),
      image: String(card?.image || '').trim()
    }))
    : [{
      scene: 1,
      goal: chapterGoal,
      conflict: chapterHook,
      turn: cliffhanger || `Escalate pressure for chapter ${chapter + 1}.`,
      image: ''
    }];
  ...
  return {
    architect: {
      ...
      theme_progress: String(mergedPlan.theme_progress || '').trim(),
      arc_targets: arcTargets,
      next_arc_targets: nextArcTargets.length ? nextArcTargets : nextChapterSeed,
      recommended_beats: toStringList(mergedPlan.recommended_beats),
      constraints: toStringList(mergedPlan.constraints),
      outline_notes: outlineNotes,
      risks: toStringList(mergedPlan.risks),
      handoff_for: 'character_master',
      _meta: {
        agent: 'architect',
        role: 'Story Architect',
        model: mergedPlan?._meta?.model || 'shared-fast-planner',
        synthetic: true,
        sourceAgent: mergedPlan?._meta?.agent || 'chapter_planner'
      }
    },
    chapterPlanner: {
      type: 'plan',
      chapter_title: String(mergedPlan.chapter_title || `Chapter ${chapter}`).trim(),
      chapter_hook: chapterHook,
      arc_payoffs: toStringList(mergedPlan.arc_payoffs),
      next_chapter_seed: nextChapterSeed.length ? nextChapterSeed : nextArcTargets,
      scene_cards: sceneCards,
      cliffhanger: cliffhanger || sceneCards.at(-1)?.turn || '',
      outline_memory_updates: appendUniqueStrings([], [
        ...toStringList(mergedPlan.outline_memory_updates),
        ...toStringList(mergedPlan.outline_notes)
      ]),
      handoff_for: 'writer',
...
export function buildFastCharacterMaster(bookBible = {}, architect = {}) {
  const characters = Object.entries(bookBible.characters || {});
  const architectConstraints = toStringList(architect.constraints);
  const architectRisks = toStringList(architect.risks);
  const architectBeats = toStringList(architect.recommended_beats);
  const emotionalTargets = appendUniqueStrings([], [
    ...(bookBible.themes || []).slice(0, 3),
    architect.theme_progress,
    ...characters.flatMap(([, details]) => [details?.wound, details?.arc])
  ]).slice(0, 4);

  const characterNotes = characters.slice(0, 4).map(([name, details]) => ({
    name,
    desire: details?.arc || architect.chapter_goal || 'Survive the chapter pressure.',
    fear: details?.wound || architectRisks[0] || 'Losing control.',
    contradiction: details?.role && details?.arc
      ? `${details.role} pulled toward ${details.arc}`
      : details?.arc || details?.role || architect.theme_progress || 'Stay emotionally unstable under pressure.',
    secret_pressure: toStringList(details?.secrets)[0] || architectConstraints[0] || ''
  }));

  if (!characterNotes.length) {
    characterNotes.push({
      name: 'Protagonist',
      desire: architect.chapter_goal || 'Push the story forward.',
      fear: architectRisks[0] || 'Failing under pressure.',
      contradiction: architect.theme_progress || 'Needs emotional pressure instead of clean certainty.',
      secret_pressure: architectConstraints[0] || ''
    });
  }

  const dialogueInstructions = appendUniqueStrings([], [
    ...characterNotes.map((note) => `Let ${note.name}'s dialogue reveal ${note.contradiction}.`),
    architect.theme_progress ? `Keep the subtext orbiting ${architect.theme_progress}.` : '',
    architectConstraints[0] ? `Do not break this constraint in dialogue or action: ${architectConstraints[0]}.` : ''
  ]).slice(0, 4);

  const conflictUpgrades = appendUniqueStrings([], [
    ...toStringList(architect.arc_targets),
  ...
  return {
    type: 'proposal',
    emotional_targets: emotionalTargets.length ? emotionalTargets : ['pressure', 'escalation'],
    character_notes: characterNotes,
    dialogue_instructions: dialogueInstructions,
    conflict_upgrades: conflictUpgrades,
    handoff_for: 'chapter_planner',
    _meta: {
      agent: 'character_master',
...
export function buildSkippedReviewBundle(writer, chapterPlanner = {}, reason = 'Fast mode skips deep review by default.', existing = {}) {
  const skippedReason = String(reason || 'Fast mode skips deep review by default.').trim();
  const existingSummary = existing.votes?.summary || {};
  ...
  return {
    ...
    votes: {
      ...
      summary: {
        approvals: Number(existingSummary.approvals || 0),
        revises: Number(existingSummary.revises || 0),
        rejects: Number(existingSummary.rejects || 0),
        averageScore: Number(existingSummary.averageScore || 1),
        approved: true,
        blockingVotes: existingSummary.blockingVotes || [],
        skipped: true,
...
function takeFirstItems(items, maxItems) {
...
function compactCharacterContext(details = {}) {
  const compact = {};

  for (const [key, value] of Object.entries(details || {})) {
    if (Array.isArray(value)) {
      compact[key] = value.slice(0, AGENT_CONTEXT_LIMITS.characterLists);
      continue;
    }

    if (value && typeof value === 'object') {
      compact[key] = Object.fromEntries(
        Object.entries(value).slice(0, AGENT_CONTEXT_LIMITS.characterLists)
      );
      continue;
    }
...
function compactChapterContextEntry(entry = {}) {
  ...
  return {
    ...
    unresolved_threads: toStringList(source.unresolved_threads).slice(0, AGENT_CONTEXT_LIMITS.timelineLists),
    arc_targets: toStringList(source.arc_targets).slice(0, AGENT_CONTEXT_LIMITS.timelineLists),
    next_arc_targets: toStringList(source.next_arc_targets).slice(0, AGENT_CONTEXT_LIMITS.timelineLists)
  };
}
...
function compactTimelineEvent(event) {
  ...
  if (event && typeof event === 'object') {
    return {
      ...
      summary: String(event.summary || event.event || event.title || '').trim()
    };
  }
...
function buildCompactBookBibleForAgents(bookBible = {}) {
  const chapterRegistry = takeLastItems(bookBible.chapter_registry, AGENT_CONTEXT_LIMITS.chapterRegistry)
    .map((entry) => compactChapterContextEntry(entry));
  const characters = Object.fromEntries(
    Object.entries(bookBible.characters || {})
      .slice(0, AGENT_CONTEXT_LIMITS.characters)
      .map(([name, details]) => [name, compactCharacterContext(details)])
  );
  ...
  return {
    ...
    themes: takeFirstItems(bookBible.themes, AGENT_CONTEXT_LIMITS.themes),
    world_rules: takeFirstItems(bookBible.world_rules, AGENT_CONTEXT_LIMITS.worldRules),
    characters,
    chapter_registry: chapterRegistry,
    carry_over_threads: takeLastItems(bookBible.carry_over_threads, AGENT_CONTEXT_LIMITS.carryOverThreads),
    automation_notes: takeLastItems(bookBible.automation_notes, AGENT_CONTEXT_LIMITS.automationNotes),
    _meta: {
      compact: true,
      totalChapterRegistry: Array.isArray(bookBible.chapter_registry) ? bookBible.chapter_registry.length : 0,
      totalCharacters: Object.keys(bookBible.characters || {}).length
    }
  };
}

function buildCompactTimelineForAgents(timeline = {}) {
  const chapters = takeLastItems(timeline.chapters, AGENT_CONTEXT_LIMITS.timelineChapters)
    .map((entry) => compactChapterContextEntry(entry));
  const events = takeLastItems(timeline.events, AGENT_CONTEXT_LIMITS.timelineEvents)
    .map((event) => compactTimelineEvent(event))
    .filter((event) => {
      if (typeof event === 'string') {
        return Boolean(event.trim());
      }

      return Boolean(event?.summary || event?.chapter !== null);
    });
  ...
  return {
    ...
    _meta: {
      compact: true,
      totalChapters: Array.isArray(timeline.chapters) ? timeline.chapters.length : 0,
      totalEvents: Array.isArray(timeline.events) ? timeline.events.length : 0
    }
  };
}

function buildCompactStyleGuideForAgents(styleGuide = {}) {
  const { recent_adjustments: recentAdjustments, ...rest } = styleGuide || {};

  return {
    ...rest,
    recent_adjustments: takeLastItems(recentAdjustments, AGENT_CONTEXT_LIMITS.recentAdjustments),
    _meta: {
      compact: true,
      totalRecentAdjustments: Array.isArray(recentAdjustments) ? recentAdjustments.length : 0
    }
  };
}
...
export function buildChapterBaseInput(state, chapter, options = {}) {
  ...
  const compactBookBible = buildCompactBookBibleForAgents(state.bookBible);
  const compactTimeline = buildCompactTimelineForAgents(state.timeline);
  const compactStyleGuide = buildCompactStyleGuideForAgents(state.styleGuide);
  const lastChapter = compactTimeline.chapters.at(-1) || null;
  ...
  return {
    ...
    BOOK_BIBLE: compactBookBible,
    TIMELINE: compactTimeline,
    STYLE_GUIDE: compactStyleGuide,
