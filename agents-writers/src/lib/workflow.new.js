...
import { createSeriesBook, loadSeriesBundle, saveSeriesBundle, selectActiveBook } from './series-memory.js';
import {
  appendUniqueStrings,
  deepMerge,
  loadEnv,
...
export async function initBook(rootPath, options = {}) {
  ...
  const nextBookBible = {
    ...
    themes: options.theme?.length ? options.theme : state.bookBible.themes || [],
...
export async function createBook(rootPath, options = {}) {
  ...
  if (options.title || options.genre || options.premise || options.theme?.length) {
...
export async function getStatus(rootPath) {
  ...
  return {
    ...
    outline: {
      ...
      arcMetrics: state.outlineMemory.arc_metrics || {}
    },
...
async function loadState(rootPath) {
  const defaults = buildDefaultMemorySet();
  const [agentsConfig, routing, presets, agentState, seriesBundle] = await Promise.all([
...
function buildNextOutlineMemory(chapter, payload) {
  ...
  return {
    ...
    arc_metrics: applyArcMetrics(payload.outlineMemory.arc_metrics || {}, chapter, {
...
function determineStoryPhase(chapter) {
  if (chapter <= 3) return 'opening';
  if (chapter <= 9) return 'escalation';
  if (chapter <= 14) return 'crisis';
  return 'endgame';
}
...
function applyArcMetrics(existingMetrics, chapter, context) {
  ...
  const seenArcs = new Set([...Object.keys(existingMetrics || {}), ...(context.touchedArcs || []), ...(context.nextArcs || [])]);
