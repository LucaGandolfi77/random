...
def get_settings() -> Settings:
    database_value = os.getenv("DATABASE_PATH", "./data/life_os.db")
    database_path = Path(database_value)
    if not database_path.is_absolute():
        database_path = (BASE_DIR / database_path).resolve()

    return Settings(
        app_name=os.getenv("APP_NAME", "Life Operating System"),
        app_env=os.getenv("APP_ENV", "development"),
        database_path=database_path,
        openrouter_api_key=os.getenv("OPENROUTER_API_KEY") or None,
        openrouter_model=os.getenv("OPENROUTER_MODEL") or None,
        openrouter_profile=os.getenv("OPENROUTER_PROFILE", "low-cost"),
        openrouter_base_url=os.getenv("OPENROUTER_BASE_URL", "https://openrouter.ai/api/v1"),
        openrouter_site_url=os.getenv("OPENROUTER_SITE_URL", "http://localhost:8000"),
        openrouter_app_title=os.getenv("OPENROUTER_APP_TITLE", "Life Operating System"),
