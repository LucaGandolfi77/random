...
class ChatApp:
    ...
    def load_gguf(self):
        ...
        if file_path:
            self.log(f"[*] Caricamento GGUF: {os.path.basename(file_path)}...")
            try:
                self.model = Llama(model_path=file_path, n_ctx=2048)
                self.model_type = "gguf"
                self.lbl_status.config(text=f"Modello: {os.path.basename(file_path)}", fg="green")
    ...
    def load_mlx(self):
        ...
        if dir_path:
            self.log(f"[*] Caricamento MLX da: {dir_path}...")
            try:
                self.model, self.tokenizer = load(dir_path)
                self.model_type = "mlx"
                self.lbl_status.config(text=f"Modello MLX: {os.path.basename(dir_path)}", fg="green")
